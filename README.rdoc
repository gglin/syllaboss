...
- Email invites for both students and admins - whitelist (append query string to log in)

- nested form or query string for uploading attachments
- PUSH TO DIGITAL OCEAN!!!

- ANNOUNCEMENTs notification box working on page (wow features)
...
- infinite scroll for discussion stream
...
- landing page
- homework only shows up after 6pm that day
- flow for deleting materials from school day view
- SEMESTER model
...
- make first time show up as separate column if no timestamp
- remove time column if there is no time in schedule

- sidebar not fixed when expanded (jQuery)
- javascript for coloring calendar
