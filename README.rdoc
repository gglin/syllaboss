...
** Sign In Related => X Chris
-delete username cross the whole site. X
- sign in with email X
- fix view (eventually landing page) --:> Next Week

** Nav Bar => Katie
-make a new: + in front of icons

** Side Bar => X George 
- fixed side bar until click drop down

** Day New => Jen 
-create new button (decrease size) below forms
- use query string for cancel -- redirect back to form
- auto select for other models (right now just HW working)

** search
- names of columns/models broken.


** Discussion => George
- @ functionality eventually click name goes to profile or comment thread
- students should not be able to comment on something they can't like future comments
- make comments editable
- reply button changes to cancel on dropdown
- @ashley williams --- make LINK

**General
-Change titles of pages w/ railscast X
-all forms indicate what is required
-img upload for profile w/ carrierwave
-fix iframe formatting

**next week
- user show page (profiles)
- index user view (but user able to edit their own information)

-First view would be week -> set that shit up today
-Render in IFrame without layout
